...
export default class NotRobot extends Button {
    ...
    async run(ctx, bot) {
        ...
        if (!channel?.autoApprove && channel?.greeting) {
            if(!user) {
                const newUser = new User({_id: ctxUser.user.id, username: ctxUser.user.username, channels:[user.request.channel]});
                await newUser.save()
            } else {
                user.channels = [...user.channels, user.request.channel]
                await user.save()
            }
