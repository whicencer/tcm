...
export default class EditCreatedImageGreeting extends Button {
    constructor() {
        super({name: "edit-created-image-greeting"});
    }
    ...
    async run(ctx, bot) {
        ...
        await ctx.api.editMessageText(ctx.update.callback_query.message.chat.id, ctx.update.callback_query.message.message_id, "❔ Вышлите изображение для приветствия");
