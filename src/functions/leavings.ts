...
export default class Leavings extends Command {
    constructor() {
        super({
            name: ["leavings 1", "leavings 2", "leavings 3", "leavings 4", "leavings 5", "leavings 6"],
        });
    }
        ...
        if (admin.command === "leavings 1") {
            if (typeof admin.value === "string" && admin.value.length >= 1) {
                ...
                if (ctx.message.entities || ctx.message.caption_entities) {
                    ...
                    channel.leaving = [...channel.leaving, {text: admin.value, entities}];
                    await channel.save();
                } else {
                    channel.leaving = [...channel.leaving, {text: admin.value}];
                    await channel.save();
                }
                ...
                if (ctx.message?.photo || ctx.message?.video || ctx.message?.animation) {
                    ...
                    if (updates.length > 1) {
                        ...
                        channel.leaving = [...channel.leaving.filter((val, index) => index !== channel.leaving.length - 1), {image: images, ...channel.leaving[channel.leaving.length - 1],},];
                        await channel.save();
                    } else {
                        channel.leaving = [...channel.leaving.filter((val, index) => index !== channel.leaving.length - 1), {image: ctx.message?.photo ? `${ctx.message.photo[0].file_id}^photo` : ctx.message?.video ? `${ctx.message.video.file_id}^video` : `${ctx.message.animation.file_id}^gif`, ...channel.leaving[channel.leaving.length - 1],},];
                        await channel.save();
                    }
                }
                ...
                if (channel.leaving[channel.leaving.length - 1]?.buttons.length) {
                    channel.leaving[channel.leaving.length - 1]?.buttons.forEach((item) => {
            ...
            if (typeof admin.value === "string" && admin.value.length >= 1) {
                ...
                if (ctx.message.entities || ctx.message.caption_entities) {
                    ...
                    channel.leaving = [...channel.leaving.filter((val, index) => index !== channel.leaving.length - 1), {
                        text: admin.value,
                        entities, ...channel.leaving[channel.leaving.length - 1]
                    }];
                    await channel.save();
                } else {
                    channel.leaving = [...channel.leaving.filter((val, index) => index !== channel.leaving.length - 1), {text: admin.value, ...channel.leaving[channel.leaving.length - 1]}];
                    await channel.save();
                }
        ...
        if (admin.command === "leavings 4") {
            ...
            if ((currentLeave.image && typeof currentLeave.image === "string") || (!currentLeave.image)) {
                ...
                if (!buttons.includes("error")) {
                    channel.leaving[channel.leaving.length - 1].buttons = [...currentLeave.buttons, ...buttons,];
                    await channel.save();
